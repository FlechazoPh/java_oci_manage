...
> 终端基于 xterm.js 6.0，采用 WebGL 渲染（不支持的环境自动回退 DOM），大量输出更流畅；搜索为官方插件，支持全匹配高亮、概览标尺标记与命中计数。
...
右键粘贴多行命令时，整段文本会停在输入行、**不会自动执行**，需要再按一次回车才运行（自动去除结尾换行 + bracketed paste 协商），避免误粘贴脚本直接执行；单行粘贴保持原有自动执行行为。远端未开启括号粘贴时会给出轻提示。
...
上传 / 下载任务统一汇总到独立的**传输管理器面板**：

- 并发多任务逐项进度条，实时显示速度与百分比
- 可折叠为 pill、保留最近完成历史、支持取消单个或全部传输
- 分屏时各面板的传输状态相互隔离，互不串台

### 无 SFTP 主机自动降级

连接 OpenWrt / dropbear / busybox 等**没有 SFTP 子系统**的主机时，文件面板会自动降级到 exec 通道，浏览、上传、下载、新建目录、删除、在线编辑照常可用，全程无感知；面板顶部会显示兼容模式横幅提示当前处于降级传输。
...
给主机卡片打标签，快速分组和过滤大量会话：

- **打标签 / 移除** — 卡片上内联输入标签名回车即添加，点击标签的 × 即移除（单主机最多 12 个标签，单个标签最长 24 字）
- **标签筛选栏** — 主机面板顶部提供标签筛选行，多选标签为「交集 (AND)」过滤并显示命中计数
- **展开 / 收起 / 清除** — 标签较多时可折叠，支持一键清除全部筛选
- **搜索兼容标签** — 搜索框同时匹配会话名、IP 和标签
...
| **AWS** | EC2 实例与 Lightsail 轻量实例的公网/私有 IP、名称、机型/套餐、区域 |
