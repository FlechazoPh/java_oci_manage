...
- 获取 AWS 云操作键盘，可管理 EC2 实例和 Lightsail 轻量实例

### /gcp

- 获取 GCP 云操作键盘来进行 Compute Engine 实例管理等操作
...
首次安装客户端后，复制页面提示的 `/bindclient 用户名 密码` 命令发送给机器人，完成客户端激活
...
当前账户可进行开机操作,自定义的系统、cpu类型你需要知道甲骨文内部名称点击的是会有链接提示你去看aarch为arm类型，延迟单位为秒，需要注意的是开机时需要二次确认完成再次点击键盘确定开机按钮才开始进行开机操作
...
设置当前客户端指定监控ip来触发换ip的动作并进行域名绑定，选择的ip，选择是否需要代理(无代理则会使用当前客户端ip为基准进行判断)，最后奈飞非自制剧检测 即可进行监测;支持指定ip段功能
